...
//shortText = jQuery.trim(title).substring(5).trim(this);
//alert(shortText);
...
ScrollAnimator = function() {
	...
	function animationLoop() {
		...
		scTop = scrollTop;
		$("#myStatus").text(scrollTop+" : "+$(document).width());
		highlighting();
...
//		$("#myStatus").text(scrollTop);
	...
	function init( opts ) {
		...
		if (touch) {
			...
			var touchcontrol = settings.touchcontrol[0];
			
			touchcontrol.addEventListener('touchstart', touchStartHandler, true);
			touchcontrol.addEventListener('touchmove', touchMoveHandler, true);
			touchcontrol.addEventListener('touchend', touchEndHandler, true);
...
//			return;
		...
		autoScrollInterval = setInterval( prm, 100  );
	}
